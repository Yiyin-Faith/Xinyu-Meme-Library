...
if (!debugBuild && hasPartialSigning) {
...
let signingConfig;
if (debugBuild) {
  // Gradle creates and uses the developer's local debug key without release credentials.
  signingConfig = undefined;
} else if (hasConfiguredSigning) {
...
const args = [debugBuild ? 'assembleDebug' : 'assembleRelease', '--console=plain', '--no-daemon', '--max-workers=2', '-Dorg.gradle.internal.http.connectionTimeout=15000', '-Dorg.gradle.internal.http.socketTimeout=15000', '-Dhttps.protocols=TLSv1.2', '-Djdk.tls.client.protocols=TLSv1.2', '-Dhttps.proxyType=HTTP'];
...
if (result.status === 0) {
  ...
  const apkPath = `release/Android/Xinyu-Meme-Library-${version}-Android${debugBuild ? '-debug' : ''}.apk`;
  ...
  if (debugBuild) {
    ...
    env.XINYU_ANDROID_STORE_PASSWORD = signingConfig.keystorePassword;
    env.XINYU_ANDROID_KEY_PASSWORD = signingConfig.keyPassword;
    const signingArgs = ['sign', '--ks', signingConfig.keystore, '--ks-key-alias', signingConfig.keyAlias, '--ks-pass', 'env:XINYU_ANDROID_STORE_PASSWORD', '--key-pass', 'env:XINYU_ANDROID_KEY_PASSWORD', '--out', signedPath, unsignedPath];
  ...
  console.log(`APK signed and verified with ${debugBuild ? 'local Gradle debug key (development only)' : signingConfig.source}.`);
